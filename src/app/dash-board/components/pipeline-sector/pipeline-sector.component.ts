...
@Component({
  selector: 'app-pipeline-sector',
  templateUrl: './pipeline-sector.component.html',
  styleUrls: ['./pipeline-sector.component.scss'],
  providers: [TranslatePipe]
})
export class PipelineSectorComponent implements OnInit {
  ...
  async serviceCall(request: any) {
    ...
    if (data.response.Message !== 'Success') {
      ...
      this.initChart(_data, reportData);
    }

  }

  initChart(chartData: any, reportData: any) {
    // Top sector download we taking top 10 values
    let topSectors = chartData.responseData.slice(0, 10);
    this.downloadData = {
      responseData: topSectors
    }
    // get year & request type wise values
    let pay_completed: any;
    let pay_partially_completed: any;
    let yet_2_pay;
    if (this.selectedVal === this.options[0].info) {
      pay_completed = reportData.responseData.map(data => this.dashboardService.kFormatter(data['pay_completed'])).slice(0, 10);
      pay_partially_completed = reportData.responseData.map(data => this.dashboardService.kFormatter(data['pay_partially_completed'])).slice(0, 10);
      yet_2_pay = reportData.responseData.map(data => this.dashboardService.kFormatter(data['yet_2_pay'])).slice(0, 10);
    } else {
      pay_completed = reportData.responseData.map(data => data['pay_completed']).slice(0, 10);
      pay_partially_completed = reportData.responseData.map(data => data['pay_partially_completed']).slice(0, 10);
      yet_2_pay = reportData.responseData.map(data => data['yet_2_pay']).slice(0, 10);
    }
    console.log(chartData);
    let label = chartData.responseData.map(data => data['label'] = `${data['source']} - ${data['destination']}`).slice(0, 10);
    this.loader = false;
    let titleText = this.translate.transform("Pipeline Revenue – Top Sectors");
    this.chartOptions = {
      series: [
        {
        ...
        toolbar: {
          ...
          export: {
            csv: {
              ...
              filename: 'Pipeline Revenue - Top sector',
            },
            svg: {
              filename: 'Pipeline Revenue - Top sector'
            },
  ...
  public downloadChart(type: any) {
    ...
    this.dashboardDownload.chartDownload(this.downloadData.responseData, type, legendItems, 'Pipeline revenue - top sector', 'pipelineSectorDiv', this.downloadHeader)
  }
}
